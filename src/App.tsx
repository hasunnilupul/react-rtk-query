...
function App() {
  ...
  const [editingContact, setEditingContact] = useState<Contact>({ id: "", name: "", "email": "" });
  ...
  return (
    <>
      ...
      <main className='relative flex-grow flex flex-col justify-between items-center'>
              ...
              <li key={contact.id} className="flex min-w-0 gap-x-4 py-5">
                ...
                <div className="min-w-0 flex-auto">
                  <p className="text-sm font-semibold leading-6 text-gray-900">
                    {contact.name}
                    <button onClick={(e) => openEditModal(contact)} className="inline-flex w-full justify-center rounded-md text-gray-600 text-sm font-semibold hover:text-gray-500 sm:ml-3 sm:w-auto">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
                      </svg>
                    </button>
                    <button onClick={(e) => handleOnDelete(contact.id)} className="inline-flex w-full justify-center rounded-md text-red-600 text-sm font-semibold hover:text-red-500 sm:ml-3 sm:w-auto">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        ...
        <UpdateContact open={showEditModal} setOpen={closeEditModal} contact={editingContact} />
      </main>
    </>
  );
}
